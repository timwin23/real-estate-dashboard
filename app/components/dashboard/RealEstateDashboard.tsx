...
const getRateColor = (title: string, rate?: number): string => {
    ...
    switch (title) {
        case 'OUTBOUND': return value >= 5 ? 'text-green-400' : value >= 3 ? 'text-yellow-400' : 'text-red-400';
        case 'TRIAGE': return value >= 50 ? 'text-green-400' : value >= 30 ? 'text-yellow-400' : 'text-red-400';
        case 'FOLLOW UPS': return value >= 50 ? 'text-green-400' : value >= 30 ? 'text-yellow-400' : 'text-red-400';
        case 'APPOINTMENTS': return value >= 80 ? 'text-green-400' : value >= 70 ? 'text-yellow-400' : 'text-red-400';
        case 'CONTRACTS': return value >= 50 ? 'text-green-400' : value >= 30 ? 'text-yellow-400' : 'text-red-400';
        case 'CLOSES': return value >= 30 ? 'text-green-400' : value >= 20 ? 'text-yellow-400' : 'text-red-400';
...
export default function RealEstateDashboard() {
    ...
    const [marketingData, setMarketingData] = useState<RawData[]>([]);
    ...
    const teamMembers = [
        { id: 'ALL' as TeamMemberKey, name: 'All Members' },
        { id: 'CHRIS' as TeamMemberKey, name: 'Chris' },
        { id: 'ISRAEL' as TeamMemberKey, name: 'Israel' },
    ...
    const calculateCurrentLevel = () => Math.floor(getCurrentXP() / 2000) + 1;
    const progressToLevel25 = Math.min((getCurrentXP() / nextLevelXP) * 100, 100);

    const calculateStreak = (data: TeamMemberData[], projections: TeamProjections | null) => {
        if (!data?.length || !projections?.CHRIS?.outbound?.daily) return 0;
        ...
        for (const entry of sortedData) {
            if (entry.outbound >= target) streak++;
            else break;
        }
    ...
    const formatDataForBarChart = (data: TeamMemberData[]) => {
        if (!data?.length) return { daily: {}, weekly: {}, monthly: {} };

        const dailyData = data[data.length - 1] || {};
        const weeklyData = data.slice(-7).reduce((acc, curr) => ({
            outbound: (acc.outbound || 0) + (curr.outbound || 0),
            triage: (acc.triage || 0) + (curr.triage || 0),
            followUps: (acc.followUps || 0) + (curr.followUps || 0),
            appointments: (acc.appointments || 0) + (curr.appointments || 0),
            shows: (acc.shows || 0) + (curr.shows || 0),
            contracts: (acc.contracts || 0) + (curr.contractsSigned || 0),
            closes: (acc.closes || 0) + (curr.closes || 0),
        }), {} as ChartData);
    ...
    useEffect(() => {
        async function loadData() {
            try {
                ...
                const [rawData, projectionsData] = await Promise.all([
                    fetchRawData(),
                    fetchProjections()
                ]);
                ...
                if (dateRange === 'ALL') {
                    ...
                    setCurrentStreak(calculateStreak(filteredSalesData, projectionsData));
                }
            } catch (error) {
